...
const peerReviewSchema = z.object({
  image_url: z.string().min(1, '이미지 URL을 입력해주세요'),
...
export default function AdminPeerReviewsPage() {
  const { data: peerReviews, isLoading, mutate } = usePeerReviews()
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<PeerReview | null>(null)
  const [deletingItem, setDeletingItem] = useState<PeerReview | null>(null)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isDeleting, setIsDeleting] = useState(false)

  const { register, handleSubmit, reset, formState: { errors } } = useForm<PeerReviewFormData>({
  ...
  const handleAddClick = () => {
    ...
    reset({
      ...
      description: '',
      year: new Date().getFullYear(),
      sort_order: peerReviews ? peerReviews.length : 0,
    })
    setIsDialogOpen(true)
  }

  const handleEditClick = (item: PeerReview) => {
    setEditingItem(item)
    reset({
  ...
  const handleDeleteClick = (item: PeerReview) => {
  ...
  const onSubmit = async (data: PeerReviewFormData) => {
    ...
    try {
      ...
      if (editingItem) {
        await apiClient.put(`/peer-reviews/${editingItem.id}`, payload)
        toast.success('동료평가가 성공적으로 수정되었습니다')
      } else {
        await apiClient.post('/peer-reviews', payload)
        toast.success('동료평가가 성공적으로 추가되었습니다')
      }
      ...
      toast.error(editingItem ? `동료평가 수정에 실패했습니다: ${errorMessage}` : `동료평가 추가에 실패했습니다: ${errorMessage}`)
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true)
    try {
      await apiClient.delete(`/peer-reviews/${deletingItem.id}`)
      toast.success('동료평가가 성공적으로 삭제되었습니다')
      await mutate()
      ...
      const errorMessage = error && typeof error === 'object' && 'message' in error
        ? (error as { message: string }).message : '알 수 없는 오류가 발생했습니다'
      toast.error(`동료평가 삭제에 실패했습니다: ${errorMessage}`)
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  return (
    <>
      <AdminHeader
        title="동료평가 관리"
        description="동료평가 이미지를 추가/수정/삭제합니다"
        action={
          <Button variant="neon" size="default" onClick={handleAddClick} className="gap-2">
            <Plus className="size-4" />새 동료평가 추가
          </Button>
        }
      />

      {isLoading ? (
        <PeerReviewsTableSkeleton />
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-[--color-neon-cyan-800] hover:bg-[--color-neon-cyan-500]/5">
                <TableHead className="text-[--color-neon-cyan-600]">이미지</TableHead>
            ...
            <TableBody>
              {peerReviews.sort((a, b) => b.year - a.year || a.sort_order - b.sort_order).map((item) => (
                <TableRow key={item.id} className="border-b border-[--color-neon-cyan-900] transition-colors hover:bg-[--color-neon-cyan-500]/5">
                  <TableCell className="text-[--color-neon-cyan-300]">
                    <div className="flex items-center gap-2">
                      <ImageIcon className="size-4 text-[--color-neon-cyan-600]" />
                      <span className="font-mono text-xs truncate max-w-[200px]">{item.image_url}</span>
                    </div>
                  </TableCell>
        ...
        <div className="rounded-lg border border-[--color-neon-cyan-800] bg-[--color-black-elevated] p-12 text-center shadow-[0_0_15px_rgba(0,240,255,0.1)]">
          <ImageIcon className="mx-auto mb-4 size-12 text-[--color-neon-cyan-700]" />
          <h3 className="mb-2 text-lg font-semibold text-[--color-neon-cyan-600]">동료평가가 없습니다</h3>
          <p className="mb-6 font-mono text-sm text-[--color-neon-cyan-700]">새 동료평가를 추가해주세요.</p>
          <Button variant="neon" size="default" onClick={handleAddClick} className="gap-2">
            <Plus className="size-4" />첫 동료평가 추가
          </Button>
        </div>
      )}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="border-[--color-neon-cyan-700] !bg-[#0a0a0a] opacity-100 backdrop-blur-none shadow-[0_0_30px_rgba(0,240,255,0.2)] sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-[--color-neon-cyan-500]">{editingItem ? '동료평가 수정' : '새 동료평가 추가'}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="image_url" className="text-[--color-neon-cyan-600]">이미지 URL <span className="text-[--color-neon-orange-500]">*</span></Label>
              <Input id="image_url" {...register('image_url')} placeholder="https://..." className="border-[--color-neon-cyan-800] bg-[--color-black-surface] text-[--color-neon-cyan-300]" />
              {errors.image_url && <p className="font-mono text-xs text-[--color-neon-orange-500]">{errors.image_url.message}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="thumbnail_url" className="text-[--color-neon-cyan-600]">썸네일 URL <span className="ml-2 font-mono text-xs text-[--color-neon-cyan-800]">(선택사항)</span></Label>
              <Input id="thumbnail_url" {...register('thumbnail_url')} placeholder="https://..." className="border-[--color-neon-cyan-800] bg-[--color-black-surface] text-[--color-neon-cyan-300]" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="description" className="text-[--color-neon-cyan-600]">설명 <span className="ml-2 font-mono text-xs text-[--color-neon-cyan-800]">(선택사항)</span></Label>
              <Textarea id="description" {...register('description')} rows={3} placeholder="동료평가 내용" className="border-[--color-neon-cyan-800] bg-[--color-black-surface] text-[--color-neon-cyan-300]" />
            </div>
            <div className="space-y-2">
      ...
      <DeleteConfirmDialog
        ...
        title="동료평가를 삭제하시겠습니까?"
        description={deletingItem ? `이 항목이 영구적으로 삭제됩니다.` : '이 작업은 되돌릴 수 없습니다.'}
...
function PeerReviewsTableSkeleton() {
  return (
