...
import { useActivities } from '@/lib/hooks/use-portfolio-data'
...
export default function AdminActivitiesPage() {
  const { data: activities, isLoading, mutate } = useActivities()
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<ExternalActivity | null>(null)
  const [deletingItem, setDeletingItem] = useState<ExternalActivity | null>(null)
  ...
  const handleAddClick = () => {
    setEditingItem(null)
    reset({ organization: '', description: '', sort_order: activities ? activities.length : 0 })
    setIsDialogOpen(true)
  }

  const handleEditClick = (item: ExternalActivity) => {
    setEditingItem(item)
  ...
  const onSubmit = async (data: ActivityFormData) => {
    ...
    try {
      ...
      if (editingItem) {
        await apiClient.put(`/activities/${editingItem.id}`, payload)
        toast.success('대외활동이 수정되었습니다')
      } else {
        await apiClient.post('/activities', payload)
        toast.success('대외활동이 추가되었습니다')
      }
      await mutate()
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true)
    try {
      await apiClient.delete(`/activities/${deletingItem.id}`)
      toast.success('대외활동이 삭제되었습니다')
      await mutate()
  ...
  return (
    <>
      <AdminHeader title="대외활동 관리" description="대외활동을 추가/수정/삭제합니다" action={<Button variant="neon" onClick={handleAddClick} className="gap-2"><Plus className="size-4" />새 대외활동 추가</Button>} />
      {isLoading ? <Skeleton className="h-64" /> : activities && activities.length > 0 ? (
        <div className="overflow-hidden rounded-lg border border-[--color-neon-cyan-700] bg-[--color-black-elevated] shadow-[0_0_20px_rgba(0,240,255,0.15)]">
          <Table>
            ...
            <TableBody>
              {activities.sort((a, b) => b.year - a.year).map((item) => (
                <TableRow key={item.id} className="border-b border-[--color-neon-cyan-900] hover:bg-[--color-neon-cyan-500]/5">
                  <TableCell className="text-[--color-neon-cyan-300]"><div className="flex items-center gap-2"><Users className="size-4 text-[--color-neon-cyan-600]" />{item.organization}</div></TableCell>
        ...
        <div className="rounded-lg border border-[--color-neon-cyan-800] bg-[--color-black-elevated] p-12 text-center">
          <Users className="mx-auto mb-4 size-12 text-[--color-neon-cyan-700]" />
          <h3 className="mb-2 text-lg font-semibold text-[--color-neon-cyan-600]">대외활동이 없습니다</h3>
          <Button variant="neon" onClick={handleAddClick} className="gap-2"><Plus className="size-4" />첫 대외활동 추가</Button>
        </div>
      )}

      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="border-[--color-neon-cyan-700] !bg-[#0a0a0a] shadow-[0_0_30px_rgba(0,240,255,0.2)] sm:max-w-[500px]">
          <DialogHeader><DialogTitle className="text-[--color-neon-cyan-500]">{editingItem ? '대외활동 수정' : '새 대외활동 추가'}</DialogTitle></DialogHeader>
      ...
      <DeleteConfirmDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen} onConfirm={handleDeleteConfirm} title="대외활동을 삭제하시겠습니까?" description={deletingItem ? `${deletingItem.organization} 항목이 삭제됩니다.` : ''} isDeleting={isDeleting} />
    </>
  )
}
